...
class Sensor:
    ...
    def post_data(self, url):
        headers={'token':'1234abc'}
        x = requests.post(url, data = self.__dict__, headers=headers)
        print(x.text)
...
if __name__=='__main__':
    ...
    humidity_thread = threading.Thread(name='humidity_sensor', target=post_data_loop, args=(humidity_sensor, TIME_SLEEP*2, 100)) 
    tempeature_thread = threading.Thread(name='temperature_sensor', target=post_data_loop, args=(temperature_sensor, TIME_SLEEP*1.2, 35))
    lighting_thread = threading.Thread(name='lighting_sensor', target=post_data_loop, args=(lighting_sensor, TIME_SLEEP*1.3, 1000))
